...
- 보안 강화
  - 각 컨트롤러 별로 입력 값을 제한하고, 메타로 저장되는 값을 제한 할 수 있도록 한다.
  - `controller/validator.php` 에서 각 컨트롤러를 실행하기 전에, 입력 값을 검사 할 수 있도록 한다.
      - 이 조건은 view(theme) 마다 다르게 설정 할 수 있도록 해야 한다.
  - 예) 아래의 예제에서 emailValidation, stringValidation 은 함수이고, 모두 조건을 만족해야 해당 컨트롤러가 실행된다.
      그리고 auth 에서 loginValidation 은 사용자가 로그인을 하지 않았으면, 컨트롤러로 가기 전에 에러가 발생한다.
      auth 에서 adminValidation 을 줄 수도 있다. 이렇게 하면, 관리자로 로그인을 해야지만 한다.
      validation 함수에서는 기타 컨트롤러를 수행하기 전에, 입력 값이 충족하는지 검사하기 위한 것이다.
    rules => [
      'user.login' => [
        'required' => [
          'email' => emailValidation,
          'password' => stringValidation,
          'firebaseUid' => stringValidation,
        ],
        'auth' => loginValidation, (또는 adminValidation),
...
- 관리자는 모든 정보, 다른 회원의 정보를 업데이트 할 수 있다.
- 기본적으로 회원의 데이터는 해당 회원 또는 관리지가 아니면, 수정이 안된다.
- 예를 들어, 회원 정보는 해당 회원 자신 뿐만아니라, 관리자가 다른 회원의 정보를 수정 할 수 있다.
- 뿐만아니라, 설정에 의해서, 막혀있는 정보의 경우, 회원 자신은 자신의 정보를 업데이트 하지 못하지만, 관리자는 업데이트 할 수 있다.
- 게시글/코멘트의 경우도, 해당 글쓴이 또는 관리자만 업데이트 할 수 있다. 다른 사람이 다른 글/코멘트를 수정하지 못한다.
  - 이것은 추천, 글 조회수 증가, 신고를 할 때, 해당 글/코멘트 레코드를 업데이트해야하는데, 그렇지 못하게한다.
    이는, `post()->update()` 또는 `comment()->update()` 함수에서 글/코멘트 소유자 검사를 해서 그런데, 이 함수 말고,
...
- 세션 아이디는 회원을 추적하기 위한, 또는 로그인 정보를 유지한 비밀 키이다.
- 이 세션 아이디만 알고 있으면, 해당 회원으로 로그인을 할 수 있다.
- 주의 할 점은 이 session id 는 차후, 변경이 될 수 있다. 예를 들면, 회원 정보를 업데이트하면, `updatedAt` 필드 값이 변경되는데,
  session id 가 이 `updatedAt` 필드 값을 바탕으로 회원 session id 가 업데이트 될 수 있다.
  따라서, 파이어 베이스 로그인/가입과 같은 것에 이 session id 를 비밀번호로 하면, 나중에 session_id 가 변경되면, 로그인을 하지 못 할 수 있다.
  

## 파이어베이스 로그인/회원 가입

- matrix 에 로그인을 하면, 클라이언트에서 자동으로 파이어베이스로 로그인 또는 회원 가입을 한다. (또는 해야 한다.)
  이 때, matrix 의 회원 메일 주소가 로그인 메일 주소가 되는데, 비밀번호는 `회원번호:createdAt` 으로 한다.
...
- 최대한 간단하게 작성
  ...
  - 게시판 별 금액을 따로 두지 않는다.
    - 차라리 게시판을 세분화 하고, 통합이 필요하면, 커뮤니티 전체, 비즈니스 전체를 그룹화해서 보여주도록 한다.
...
- `userAgent` 는 클라이언트 userAgent 이다.
  
